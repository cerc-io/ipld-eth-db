...
CREATE OR REPLACE FUNCTION was_state_leaf_removed(key character varying, hash character varying)
    RETURNS boolean AS $$
    SELECT state_leaf_cids.node_type = 3
    FROM eth.state_leaf_cids
             INNER JOIN eth.header_cids ON (state_leaf_cids.header_id = header_cids.block_hash)
    WHERE state_leaf_key = key
      AND state_leaf_cids.block_number <= (SELECT block_number
                           FROM eth.header_cids
                           WHERE block_hash = hash)
    ORDER BY state_leaf_cids.block_number DESC LIMIT 1;
$$
language sql;
-- +goose StatementEnd
